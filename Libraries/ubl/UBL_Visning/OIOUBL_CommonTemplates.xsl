...
******************************************************************************************************************
		...
		created= 2006-12-29
		modified= 2011-05-12 by Charlotte Dahl Skovhus
		issued= 2007-07-20
...
<xsl:stylesheet version="1.0"
	...
	<xsl:template match="cac:Party | cac:PayeeParty | cac:SenderParty | cac:ReceiverParty | cac:DeliveryParty | cac:SignatoryParty | cac:OriginatorParty">
		<div class="UBLPayeeParty">
			...
			<!--Change: Test added 2011-05-06 Charlotte Skovhus-->
	...
	<xsl:template match="cac:PartyLegalEntity">
		<div class="UBLPartyLegalEntity">
			<span class="UBLCompanyID">
				<!--Change: Test removed 06.05.2011 Charlotte Skovhus-->
	...
	<xsl:template match="cac:Delivery" mode="line">
		...
		<td class="UBLLine">
			<b><xsl:value-of select="$moduleDoc/module/document-merge/g-funcs/g[@name='Delivery']"/></b><br/>
			<xsl:if test="cbc:ActualDeliveryDate !=''">
				<b><xsl:value-of select="$moduleDoc/module/document-merge/g-funcs/g[@name='ActualDeliveryDate']"/></b>&#160;<xsl:apply-templates select="cbc:ActualDeliveryDate"/><br/>
			</xsl:if>
			<xsl:if test="cac:DeliveryLocation != ''">
				<b><xsl:value-of select="$moduleDoc/module/document-merge/g-funcs/g[@name='DeliveryLocation']"/></b><br/>
				<xsl:apply-templates select="cac:DeliveryLocation"/>
			...
			<xsl:if test="cac:RequestedDeliveryPeriod !=''">
				<xsl:apply-templates select="cac:RequestedDeliveryPeriod"/><br/>
			</xsl:if>
			<xsl:if test="cac:DeliveryParty !=''">
				<xsl:choose>
					<xsl:when test="$PersonalSecure = 'PersonalSecure' and cac:DeliveryParty/cac:PartyIdentification/cbc:ID/@schemeID = 'DK:CPR'"></xsl:when>
					<xsl:otherwise>
						<b><xsl:value-of select="$moduleDoc/module/document-merge/g-funcs/g[@name='DeliveryParty']"/></b><br/>
						<xsl:apply-templates select="cac:DeliveryParty"/>
					</xsl:otherwise>
	...
	<!--Fakturalinje herfra-->
		<xsl:template match="cac:InvoiceLine">
			<tr class="UBLInvoiceLine" valign="top">
				<td>
			...
			<tr valign="top">
				...
				<td class="UBLLine" colspan="3">
					...
					<xsl:if test="cac:Item/cbc:Description !='' and cac:Item/cbc:Name != cac:Item/cbc:Description">
						<b><xsl:value-of select="$moduleDoc/module/document-merge/g-funcs/g[@name='ItemDescription']"/>&#160;</b> <xsl:apply-templates select="cac:Item/cbc:Description"/><br/>
					</xsl:if>
				...
				<!-- Charlotte Skovhus: Changed 2011-05-11
	...
	<!--Kreditnotalinje herfra-->
		<xsl:template match="cac:CreditNoteLine">
			<tr class="UBLCreditNoteLine" valign="top">
				<td>
					<xsl:apply-templates select="cbc:ID"/>
				</td>
				<td class="UBLSellersItemIdentification">
	...
	<!--Ordrelinje herfra-->
		<xsl:template match="cac:OrderLine/cac:LineItem">
			<xsl:param name="PersonalSecure"/>
			<tr class="UBLOrderLine" valign="top">
...
<!--Referencer herfra-->
	...
	<xsl:template match="cac:Delivery/cac:RequestedDeliveryPeriod | cac:InvoicePeriod | cac:RequestedDeliveryPeriod | cac:ValidityPeriod">
		<xsl:if test="string(cbc:StartDate) or string(cbc:EndDate)">
			<b><xsl:value-of select="$moduleDoc/module/document-merge/g-funcs/g[@name='RequestedDeliveryPeriod']"/></b>&#160;
			<xsl:choose>
