...
function lookup_acc($accounts,$bal,$alias = "",$multi = "--multi") {
	...
	$accountstring = "";
	ob_start();
	$cmd = ("((updatealiases= LEDGER_BEGIN=1970-01-01 LEDGER_END=2099-12-31 noend=1 php /svn/svnroot/Applications/newl.php accounts;cat /svn/svnroot/Libraries/Kontoplan.txt)|sort|uniq|grep :;echo NY)|fzf --ansi --tac --header-first --header=\"$alias\" --scrollbar=* $multi --margin 1% --padding 1% --border --preview-label='Seneste posteringer' --preview-window 35%:bottom --preview 'LEDGER_SORT=date LEDGER_PAYEE_WIDTH=15 updatealiases= php /svn/svnroot/Applications/newl.php register ^\"{}\" |tac'");
	system($cmd);
