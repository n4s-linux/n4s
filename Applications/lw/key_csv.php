...
function loadfile() {
  global $path;
  global $accounts;
  $availablefields =array( "Date","Description","Reference","Amount","Account","Func","Choose");;
       array_push($availablefields,"No match");
$op=system("whoami");
	global $argv;
	if (!isset($argv[2]))
  $filename = "/home/$op/n4s-export/" . time() . ".csv";
	else
		$filename = $argv[2];
...
if (isset($c['Account']))
	...
	if (strtotime($curtrans['Date']) > strtotime("+1 days"))
...
$curtrans['History'] = array(array('Date'=>date("Y-m-d"),'Desc'=>'Loaded from CSV'));
                 $curtrans['Transactions'] = array(
                   ...
                   array('P-Start'=>'','P-End'=>'','Account'=> ($curacc == "") ? 
                         (( $c['Amount'] < 0) ? "Fejl/analyse:Uhåndterede kreditorbetalinger" : "Fejl/analyse:Uhåndterede debitorbetalinger") : $curacc
